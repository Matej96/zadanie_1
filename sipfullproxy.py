...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            ...
            global canceled
            canceled= False
            with open("dennik.txt", "a") as file1:
                format = "%d.%m. %H:%M:%S :: "

                file1.writelines("> " + time.strftime(format) + self.data[5] + " :: " + origin.split("@")[0] + " vola " + destination.split("@")[0] + "\n")
            ...
            self.sendResponse("500 Chyba Severa")
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            ...
            with open("dennik.txt", "a") as file1:
                ...
                file1.writelines("> " + time.strftime(format) + self.data[5] + " :: " + origin.split("@")[0] + " zrusil\n")
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("406 Nemozno Prijat")
        else:
            # self.sendResponse("500 Server Internal Error")
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                global canceled
                if data[0] == "SIP/2.0 200 Ok" and canceled is False:
                    with open("dennik.txt", "a") as file1:
                        format = "%d.%m. %H:%M:%S :: "

                        file1.writelines("> " + time.strftime(format) + self.data[5] + " :: " + self.getDestination().split("@")[0] + " prijal\n")
                        canceled = True
